...
Item {
    ...
    RegularText{
        ...
        id: infoText
        text: qsTr("点击左侧icon可查看详细信息，下方icon可签收快递")
        font.pixelSize: 15
    ...
    TextDialog {
        id: receiveDialog
        title: "签收页"
        textField.placeholderText: qsTr("需签收快递的运单号")
        onAccepted : {
            const ret = gui.receiveReq(receiveDialog.textField.value);
            if(ret) {
                error.text = "签收失败，" + ret
                error.open()
            }
